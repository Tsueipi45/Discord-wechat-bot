...
@app_commands.describe(verb="輸入一個動詞(選填)", user="選擇一個用戶")
async def pat(interaction: discord.Interaction, user: discord.Member, verb: Optional[str] = None):
    if verb is None:
        verb = "拍了拍"
    await interaction.response.send_message(f"{interaction.user.display_name} {verb} {user.mention}")
